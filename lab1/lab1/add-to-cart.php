...
file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Adding product with ID: ' . $id . " (type: " . gettype($id) . ")\n", FILE_APPEND);

// Kiểm tra xem sản phẩm đã có trong giỏ hàng chưa
$found = false;
foreach($cart as $key => $item) {
    $itemId = isset($item['id']) ? (int)$item['id'] : 0;
    file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Checking cart item with ID: ' . $itemId . " (type: " . gettype($itemId) . ")\n", FILE_APPEND);
    file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Comparing with product ID: ' . $id . " (type: " . gettype($id) . ")\n", FILE_APPEND);
    
    if($itemId == $id) {
        // Ghi log khi tìm thấy sản phẩm
        file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Found matching product ID: ' . $id . " == " . $itemId . "\n", FILE_APPEND);
        ...
        $found = true;
        file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Updated quantity to: ' . $cart[$key]['quantity'] . "\n", FILE_APPEND);
        break;
    }
}
...
if(!$found) {
    ...
    $cart[] = $newItem;
    file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Added new product to cart with ID: ' . $id . " (type: " . gettype($newItem['id']) . ")\n", FILE_APPEND);
}
...
// Hiển thị thông tin giỏ hàng để debug
file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Cart after update: ' . print_r($_SESSION['cart'], true) . "\n", FILE_APPEND);
